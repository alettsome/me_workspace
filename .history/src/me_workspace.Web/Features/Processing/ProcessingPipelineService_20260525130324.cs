    ...
    ILogger<ProcessingPipelineService> logger)
{
    ...
    {
        logger.LogInformation("ProcessSourceAsync called with sourceId: {SourceId}", sourceId);
        
        var source = await db.Sources
            .Include(s => s.Files)
            .FirstOrDefaultAsync(s => s.Id == sourceId, cancellationToken);

        logger.LogInformation("Query completed. Source found: {Found}. Source is null: {IsNull}", 
            source != null, source == null);

        if (source is null)
        {
            logger.LogWarning("Source {SourceId} not found in database", sourceId);
            return ProcessingResultDto.CreateFailure($"Source {sourceId} not found");
        }
        ...
        try
        {
            ...
            {
                return ProcessingResultDto.CreateFailure("No text content found or extracted");
            }
            ...
            return ProcessingResultDto.CreateSuccess(
        ...
        catch (Exception ex)
        {
            ...
            return ProcessingResultDto.CreateFailure($"Processing failed: {ex.Message}");
        }
    }
