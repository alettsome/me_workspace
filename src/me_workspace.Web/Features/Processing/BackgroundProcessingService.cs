...
public sealed class BackgroundProcessingService : BackgroundService
{
    ...
    private async Task<string?> ExtractPdfTextAsync(string pdfPath)
    {
        try
        {
            // Go up from src/me_workspace.Web to workspace root
            var workspaceRoot = Path.GetFullPath(Path.Combine(_environment.ContentRootPath, "..", ".."));
            var extractorPath = Path.Combine(workspaceRoot, "tools", "ExtractPdfText", "bin", "Debug", "net8.0", "ExtractPdfText.exe");
    ...
    private async Task SendNotificationAsync(string fileName, ProcessingResultDto result)
    {
        try
        {
            using var scope = _serviceProvider.CreateScope();
            ...
            var notification = new Data.Entities.ProcessingNotification
            {
                Id = Guid.NewGuid(),
                FileName = fileName,
        ...
        {
            _logger.LogError(ex, "Failed to save notification for {FileName}", fileName);
        }
    }
